...
export default function OutputRenderer({ cfg = {}, slideData, isBlank, background = {}, slideIndex, totalSlides, backgroundMedia, containerWidth = null, containerHeight = null, bgCacheKey = 0, staticVideoFrame = false, onVideoEnded = null }) {
  ...
  const isSongSlide = slideData?.isSong ?? (slideData?.type === 'song' || slideData?.type === 'title');
...
function SlideContent({ slideData, cfg, cw = null, ch = null }) {
  ...
  const isSongSlide = slideData?.isSong ?? (type === 'song' || type === 'title');
  ...
  if (type === 'song') {
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center px-16 text-center">
        ...
        {showSongTitle && isSongSlide && slideData.songTitle && (
